...
export async function generateMetadata({
  ...
  if (!project) return {};
  return {
    title: `${project.title} — Alfredo Dominguez`,
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <div className="border-b border-border-subtle sticky top-0 bg-background/90 backdrop-blur-sm z-40">
        <div className="max-w-5xl mx-auto px-6 h-14 flex items-center gap-4">
          ...
          <span className="text-sm text-muted font-mono">{project.slug}</span>
        </div>
      </div>
      ...
      <main className="max-w-5xl mx-auto px-6 py-16">
        ...
        <div className="mb-16">
          <div className="flex items-center gap-3 mb-6 flex-wrap">
            ...
            <span className="text-xs font-mono px-2 py-1 rounded-full bg-surface border border-border-subtle text-foreground">
              💻 Full Stack
            </span>
          </div>
        ...
        <Section title="Problema & Solución">
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div className="bg-surface border border-foreground/10 rounded-xl p-6">
              <p className="text-xs font-mono text-foreground/60 uppercase tracking-widest mb-3">
                Solución
              </p>
              <p className="text-foreground/80 leading-relaxed">
                {project.solution}
        ...
        {/* Cómo funciona */}
        <Section title="Implementación">
          <div className="flex flex-col gap-6">
            {project.steps.map((step) => (
              <div key={step.n} className="flex gap-5">
                <div className="shrink-0 w-8 h-8 rounded-full bg-foreground/5 border border-border-subtle flex items-center justify-center">
                  <span className="text-foreground/60 font-mono text-sm font-bold">
                    {step.n}
                  </span>
                </div>
              ...
              <div key={group.category}>
                ...
                <div className="flex flex-wrap gap-2">
                  {group.items.map((item) => (
                    <span
                      key={item}
                      className="text-sm bg-surface border border-border-subtle text-foreground px-3 py-1.5 rounded font-mono"
                    >
                      {item}
                    </span>
                  ))}
        ...
        <div className="border-t border-border-subtle pt-12 flex flex-wrap gap-3">
          <Link
            href="/#proyectos"
            className="border border-border-subtle text-foreground px-6 py-3 rounded font-semibold hover:border-foreground/40 transition-colors text-sm"
          >
